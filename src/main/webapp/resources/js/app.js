...
app.controller("PieCtrl", function($scope) {
    $scope.labels = [ "Libraries", "Monuments", "Neighborhoods" ];
    $scope.data = [ 0, 0, 0 ];
    $scope.options = {
	legend : {
	    display : true
		...
		[
			...
			function($scope, Library, Monument, Neighborhood, uiGridConstants, uiGridValidateService) {
			    ...
			    uiGridValidateService.setValidator('zipcode',
				    ...
				    function(argument) {
				      return 'A 5-digit Zipcode is required.';
				    }
				  );			    
				...
				if($scope.addForm.$valid) {
				    ...
				    if (!isValidZipcode($scope.ZipCode)) {
					    ...
					    $scope.failureMessage = 'A 5-digit Zipcode is required.';
					    return;
				    }
				...
				$scope.gridApi.edit.on.afterCellEdit($scope,
					function(rowEntity, colDef, newValue,
						...
						if (rowEntity.Type == 'Library') {
						    ...
						    // I have no choice but to call my own manual validation
						    // because the ui grid validation will always try to update
				...
				ob.neighborhoods = Neighborhood
					.query(function() {
					    // update the chart with the latest data
					    chartScopeHolder.data[2] = ob.neighborhoods.length;
					    ...
					    // use a custom comparison because the neighborhoods are in an array of objects
				...
				// clears the form. Need to use this instead of $setPristine because of the way we are doing validation
...
function isValid(myObj) {
	...
	alert('Invalid Zipcode! Field is required and must be a 5-digit Zipcode.');
	return false;
    }
    return true;
}
